...
namespace apn::dark::kuro::paint::d2d
{
	...
	struct Texter
	{
		...
		{
			// 影用のカラーエントリを取得します。
//			const auto& color_entry = pigment->text.shadow.entry;

			// 影の色をD2D形式で取得します。
			auto d2d_text_shadow_color = D2D1::ColorF(D2D1::ColorF::Black, 1.0f);

			// ブラシを作成します。
			ComPtr<ID2D1SolidColorBrush> text_shadow_brush;
			render_target->CreateSolidColorBrush(d2d_text_shadow_color, &text_shadow_brush);
			if (!text_shadow_brush) return {};

			return text_shadow_brush;
		}
		...
		int draw_text_with_blur(std::wstring& s, HDC dc, LPCRECT rc, UINT text_flags, const Pigment* pigment)
		{
			...
			// オフスクリーンビットマップを作成します。
			auto in_props = D2D1::BitmapProperties1(
				D2D1_BITMAP_OPTIONS_TARGET,
//				D2D1_BITMAP_OPTIONS_TARGET | D2D1_BITMAP_OPTIONS_CANNOT_DRAW,
				core.c_pixel_format);
			ComPtr<ID2D1Bitmap1> offscreen;
			hr = wic_device_context->CreateBitmap(
				D2D1::SizeU(iw, ih), nullptr, 0, &in_props, &offscreen);
			if (!offscreen) return 0;
/*
			// 出力ビットマップを作成します。
			auto out_props = D2D1::BitmapProperties1(
				D2D1_BITMAP_OPTIONS_TARGET,
//				D2D1_BITMAP_OPTIONS_TARGET | D2D1_BITMAP_OPTIONS_CPU_READ,
//				D2D1_BITMAP_OPTIONS_TARGET | D2D1_BITMAP_OPTIONS_CANNOT_DRAW,
				core.c_pixel_format);
			ComPtr<ID2D1Bitmap1> out_bitmap;
			hr = wic_device_context->CreateBitmapFromWicBitmap(wic_bitmap.Get(), &out_props, &out_bitmap);
			if (!out_bitmap) return 0;
*/
			// オフスクリーンビットマップに描画します。
			{
