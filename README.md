...
* `ダークモード化の設定`
	...
	* `タイトルバーをスリムにする`✏️チェックを入れるとタイトルバーとメニューバーが一体化(スリムバー化)します。
	* `D2Dでテキストを描画`✏️チェックを入れるとDirect2D(DirectWrite)を使用してテキストを描画します。
...
* AviUtl ExEdit2 beta11 https://spring-fragrance.mints.ne.jp/aviutl/
